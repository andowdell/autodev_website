...
class SccBetController(BetController):
    ...
    def bet(self, auction_obj, price, price_max, is_aggressive=False):
        ...
        data_endpoint_next_car = {
            ...
            'ccstyle': 'cars',
            self.current_price_pane_id: price_to_bet,
    ...
    def find_auction(self):
        ...
        try:
            ...
            self.current_last_date = datetime.datetime.fromtimestamp(int(last_date_timestamp)/1000)
        except:
            self.current_last_date = None

        #form_id = doc.xpath("//form")[0].attrib['name']
        #pane_id = doc.xpath("//pane[@invokeevent='click']")[0].attrib['id']
        self.bet_button_id = doc.xpath("//button[@text='Gebot abgeben']")[0].attrib['id']
        try:
            self.current_price_pane_id = doc.xpath("//formattedfield")[0].attrib['id']
            self.current_price = doc.xpath("//formattedfield")[0].attrib['value']
        except:
            self.current_price = 0
        ...
        self.last_button_left = button_left
    ...
    def _get_next_car_data(self, form_id, button_right, fixgrid, modalpopup_car, get_url_car, last=False):
        ...
        if last == True and car_data['provider_id'] != self.auction.provider_id:
            logger.warning("[BetAutomate][SCC][%s] Error while searching for auction with given provider_id" % self.auction.provider_id)
            raise Exception("[BetAutomate][SCC][%s] Error while searching for auction with given provider_id" % self.auction.provider_id)
