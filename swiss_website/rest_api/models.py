...
class Auction(models.Model):
    ...
    def photos_list(self):
        photos = self.photos.all().distinct()

        def img_name_cmp(a, b):
            if len(a.image.name) < len(b.image.name):
                return -1
            elif len(a.image.name) > len(b.image.name):
                return 1

            if a.image.name < b.image.name:
                return -1
            elif a.image.name > b.image.name:
                return 1

        photos = sorted(list(photos), key=cmp_to_key(img_name_cmp))
...
class Bet(models.Model):
    ...
    date = models.DateTimeField(
        auto_now_add=True,  # fires only at creation time
        verbose_name='Data wbicia'
    )
    ...
    auction = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name='Aukcja',
        #limit_choices_to={'published': True, 'end_date__gte': timezone.now() - timezone.timedelta(days=7)},
        #limit_choices_to={'published': True},
    )
    note = models.TextField(null=True, blank=True, default="", verbose_name='Notka')
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        verbose_name='Użytkownik',
    ...
    def is_bet_automate(self):
        scheduled = ScheduledBet.objects.filter(bet=self).count()
        return bool(scheduled)
    is_bet_automate.boolean = True
    is_bet_automate.short_description = 'Automat'

    def user_bets(self):
        link = '<a href="/admin/rest_api/bet/?q=%s" style="color:#ac0303;float:right;display:inline-block">licytacje</a>' % self.user.email
        return link
    ...
    def user_registered(self):
        ...
        try:
            user = '<a href="/admin/rest_api/userprivate/%s/change/">%s</a>' % (user.id, user)
        except Exception as e:
...
class ScheduledBet(models.Model):
    ...
    bet = models.ForeignKey(
        ...
        verbose_name='Licytacja',
        limit_choices_to={
            'auction__end_date__gte': timezone.now(),
        },
    )
    is_aggressive = models.BooleanField(default=False, editable=True, verbose_name='Automat agresywny')

    def name(self):
        user = None
        try:
            user = UserPrivate.objects.get(user=self.bet.user)
            user = '%s %s' % (user.first_name, user.last_name)
        except UserPrivate.DoesNotExist:
            pass

        try:
            user = UserBusiness.objects.get(user=self.bet.user)
        except UserBusiness.DoesNotExist:
            pass

        link = '%s %s' % (self.bet.auction.title, user)
        return link
    name.description = "Licytacja"

    def bet_date(self):
        return self.bet.auction.end_date
    bet.description = "Data"

    def user_bet_price(self):
        return self.bet.price
    user_bet_price.short_description = "Cena uż. [CHF]"

    def auction_to_end(self):
        return self.bet.auction.to_end_date()
    auction_to_end.allow_tags = True
    auction_to_end.short_description = 'Do końca'
...
def save_bet(sender, instance, **kwargs):
    ...
    if created:
        ...
        auth_token = '946a43db2e5a5078f1624d146314e653'
        account_sid = 'AC082e1f68bd1fdc0f0ff53db7e89a3b25'
